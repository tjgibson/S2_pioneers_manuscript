...
To investigate the mechanisms of chromatin binding and opening by pioneer factors, we generated stable cell lines inducibly expressing Zelda (Zld) and Grainy head (Grh) in Drosophila S2 cells (Extended Data Fig. XX), where neither factor is endogenously expressed.
We validated our wild-type (WT) S2 cells by performing RNA-seq and verifying the expression of known S2 cell markers (Extended Data Fig. 1b) [@Cherbas2011].
We confirmed that Zld and Grh RNA (Extended Data Fig. XX) and protein (Extended Data Fig. XX) were not present in WT S2 cells.
...
To control for antibody specificity, we performed additional ChIP experiments in WT cells using IgG, anti-Zld, or anti-Grh (Extended Data Fig. XX).
...
Differential expression analysis using DESeq2 [@love2014] identified XXX differentially expressed genes for Zld and XXX for Grh (Extended Data Fig. XX).
...
Previous studies have proposed that pioneer factor activity may depend on protein concentration [@Hansen2022].
...
<!--# Consider toning down this statement, as the FoxA1 region that interacts with nucleosomes (Iwafuchi 2020 Nat. Genet. paper) is outside the DBD -->.
