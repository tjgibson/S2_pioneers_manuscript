...
These cell lines allowed the inducible and tunable expression of each factor, as neither pioneer factor is normally expressed in WT S2 cells (Extended Data Fig. 1b,c,d).
...
Using differential accessibility analysis comparing Zld or Grh-expressing cells to wild-type cells, we detected `r zld_diff` differentially accessible regions for Zld (`r zld_up` increased, `r zld_down` decreased) and `r grh_diff` for Grh (`r grh_up` increased, `r grh_down` decreased) (Extended Data Fig. 3a,d).
By integrating our ChIP-seq and ATAC-seq data, we identified three classes of PF-bound sites (Fig. 1a,b).
...
Differential expression analysis using DESeq2 to compare induced cells to wild-type cells identified `r zld_diff` differentially expressed genes for Zld and `r grh_diff` for Grh (Extended Data Fig. 3b,e)[@love2014].
The genes activated by Grh were enriched for gene ontology (GO) terms related to cuticle and epithelial development, consistent with the endogenous function of Grh (Extended Data Fig. 3c)[@Nevil2017].
Genes activated by Zld expression were enriched for embryonic morphogenesis, consistent with the known function of Zld[@Nien2011], as well as imaginal disc development, in which Zld-target genes have not been well characterized (Extended Data Fig. 3f).
...
Control IP experiments in wild-type S2 cells support that these peaks are not due to nonspecific immunoprecipitation (Extended Data Fig. 1e-f).
We hypothesized that the overlapping class I binding sites may occur at active *cis*-regulatory regions in wild-type cells.
To test this, we analyzed published ChIP-seq data from S2 cells for markers of active transcription: H3K27ac, CBP, H3K4me1, H3K4me3, and H2AV[@Henriques2018; @Philip2015; @Tettey2019].
...
These data suggest that a more and stronger motifs at class III sites as compared to class II sites facilitates the ability of Zld, Grh, and Twi to open chromatin.
...
Additionally, the MNase signal in many regions did not show strongly positioned nucleosomes, likely because nucleosomes at silent regions often are not strongly positioned and show variable distributions in different cell types [@Stergachis2020; @Abdulhay2020].
...
We analyzed Zld ChIP-seq data from the early embryo ([@Harrison2011], nuclear cycle 14 embryos) and from larval neural stem cells [@Larson2021a], Grh ChIP-seq data from late embryos (16-17 hours) and wing imaginal discs (Nevil et al. 2017), and Twi ChIP-seq data from early embryos (1-3 hours) [@Ozdemir2011].
We identified many binding sites that were bound *in vivo*, but were not bound upon ectopic expression in S2 cells (Extended Data Fig. 7a,b).
...
To test the functional relevance of repressive chromatin to PF binding, we used the chemical inhibitor tazemetostat to disrupt activity of the PRC2 complex responsible for depositing H3K27me3.
Despite causing dramatic reductions in H3K27me3 levels, tazemetostat treatment did not result in global changes to Zld, Grh, or Twi binding when compared to DMSO controls (Fig. 4d-f; Extended Data Fig. 8a).
...
To identify the relationship between PF binding and chromatin accessibility, we performed ATAC-seq on cells induced with the same range in expression.
...
Chromatin occupancy can be regulated by protein-intrinsic features such as motif affinity and protein domains, and by cell-intrinsic properties such as histone modifications, PF concentration, and cell-type specific cofactors.
:::
